...
- `content/archive/`
  where accepted markdown is mirrored for reading, git history, and deploys
...
Accepted drafts are mirrored into `content/archive/`. Commit those archive files after a successful publish; fresh deploys rebuild hidden runtime state from them. Rejected drafts stay visible and return structured lint feedback. Misplaced work is moved into `content/recovered-drafts/`.
...
after a successful `submit` or `edit`, commit the changed `content/archive/**` files. do not commit `content/.state/**` or `public/_redirects`; those are regenerated by audit/build.
